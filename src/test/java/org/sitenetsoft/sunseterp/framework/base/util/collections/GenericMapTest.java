...
public class GenericMapTest {
    ...
    public static final class TestGenericMap<K, V> extends GenericMap<K, V> {
        ...
        @Override
        protected Iterator<Entry<K, V>> iterator(final boolean noteAccess) {
            incrementCallCount("iterator-" + noteAccess);
            return new IteratorWrapper<Entry<K, V>, Entry<K, V>>(proxyMap.entrySet().iterator()) {
                @Override
        ...
        @Override
        protected <KE extends K, VE extends V> void putAllIterator(Iterator<Entry<KE, VE>> it) {
            incrementCallCount("putAllIterator");
            while (it.hasNext()) {
                Entry<KE, VE> entry = it.next();
