...
<#assign productFeaturesByTypeMap = Static["org.sitenetsoft.sunseterp.applications.product.feature.ParametricSearch"].makeCategoryFeatureLists(productCategoryId, delegator)>
        ...
        <form name="createProductInCategoryCheckExistingForm" method="post" action="<@ofbizUrl>CreateProductInCategoryCheckExisting</@ofbizUrl>" style="margin: 0;">
            ...
            <#list productFeaturesByTypeMap.keySet() as productFeatureTypeId>
                ...
                <tr>
                    ...
                    <#assign findCurrenciesMap = Static["org.sitenetsoft.sunseterp.framework.base.util.UtilMisc"].toMap("uomTypeId", "CURRENCY_MEASURE")>
                    <#assign currencies = EntityQuery.use(delegator).from("Uom").where(findCurrenciesMap!).cache().queryList()! />
