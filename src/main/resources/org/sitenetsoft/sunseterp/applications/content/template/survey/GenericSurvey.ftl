...
<#macro renderSurveyQuestionInput surveyQuestionAndAppl questionFieldName>
  ...
  <#elseif "ENUMERATION" == surveyQuestionAndAppl.surveyQuestionTypeId>
    ...
    <#list enums as enum>
        ...
        <#if (formatString?has_content)>
            <#assign description = Static["org.sitenetsoft.sunseterp.framework.base.util.string.FlexibleStringExpander"].expandString(formatString, enum)/>
        <#else>
  ...
  <#elseif "GEO" == surveyQuestionAndAppl.surveyQuestionTypeId>
    ...
    <#assign geos = Static["org.sitenetsoft.sunseterp.framework.common.geo.GeoWorker"].expandGeoGroup(parentGeoId, delegator)>
    <#list geos as geo>
          <#assign selected = ''/>
        ...
        <#if (formatString?has_content)>
            <#assign description = Static["org.sitenetsoft.sunseterp.framework.base.util.string.FlexibleStringExpander"].expandString(formatString, geo)/>
        <#else>
  ...
  <#elseif "STATE_PROVINCE" == surveyQuestionAndAppl.surveyQuestionTypeId>
    <select name="${questionFieldName}">
    <#assign states = Static["org.sitenetsoft.sunseterp.framework.common.CommonWorkers"].getStateList(delegator)>
...
<table width="100%" border="0" cellpadding="2" cellspacing="0">
  ...
  <#list surveyQuestionAndAppls as surveyQuestionAndAppl>
   <#if !alreadyShownSqaaPkWithColId.contains(surveyQuestionAndAppl.getPrimaryKey())>
    ...
    <#if surveyMultiResp?has_content>
      <#assign surveyMultiRespColumnList = surveyMultiResp.getRelated("SurveyMultiRespColumn", null, Static["org.sitenetsoft.sunseterp.framework.base.util.UtilMisc"].toList("sequenceNum"), true)/>
      ...
      <#list surveyMultiRespColumnList as surveyMultiRespColumn>
        <td align="center">
          ...
          <#if surveyQuestionAndAppl.surveyMultiRespColId?has_content &&
            ...
            <#assign dummySqaaWithColId = Static["org.sitenetsoft.sunseterp.framework.base.util.UtilMisc"].removeFirst(sqaaWithColIdList)/>
  ...
  <#else>
    ...
    <#if surveyQuestionAndAppl?? && surveyQuestionAndAppl.surveyQuestionTypeId?has_content>
        <#if ("BOOLEAN" == surveyQuestionAndAppl.surveyQuestionTypeId || "CONTENT" == surveyQuestionAndAppl.surveyQuestionTypeId || "OPTION" == surveyQuestionAndAppl.surveyQuestionTypeId)>
              <#assign align = "right"/>
        </#if>
    </#if>
