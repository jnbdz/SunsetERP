...
<div id="productcomparelist" class="screenlet">
  ...
  <#assign productCompareList = Static["org.sitenetsoft.sunseterp.applications.product.product.ProductEvents"].getProductCompareList(request)/>
  <#if productCompareList?has_content>
    <table>
    <#list productCompareList as product>
      <tr>
        <td>
          ${Static["org.sitenetsoft.sunseterp.applications.product.product.ProductContentWrapper"].getProductContentAsText(product, "PRODUCT_NAME", request, "html")}
        </td>
        <td>
