...
<#escape x as x?xml>
    ...
    <fo:table-body font-size="10pt">
        ...
        <#list invoiceItems as invoiceItem>
            ...
            <#assign isItemAdjustment = Static["org.sitenetsoft.sunseterp.framework.entity.util.EntityTypeUtil"].hasParentType(delegator, "InvoiceItemType", "invoiceItemTypeId", itemType.getString("invoiceItemTypeId"), "parentTypeId", "INVOICE_ADJ")/>
            ...
            <#if itemBillings?has_content>
                <#assign itemBilling = Static["org.sitenetsoft.sunseterp.framework.entity.util.EntityUtil"].getFirst(itemBillings)>
            ...
            <#assign description = Static["org.sitenetsoft.sunseterp.applications.accounting.invoice.InvoiceWorker"].getInvoiceItemDescription(dispatcher, invoiceItem, locale)>
            ...
            <#if newShipmentId?? & newShipmentId != currentShipmentId>
                ...
                <fo:table-row height="14px">
                   <fo:table-cell number-columns-spanned="5">
                        <fo:block font-weight="bold"> ${uiLabelMap.ProductShipmentId}: ${newShipmentId}<#if issuedDateTime??> ${uiLabelMap.CommonDate}: ${Static["org.sitenetsoft.sunseterp.framework.base.util.UtilDateTime"].toDateString(issuedDateTime)}</#if></fo:block>
                   </fo:table-cell>
                </fo:table-row>
            ...
            <#if !isItemAdjustment>
                <fo:table-row height="14px" space-start=".15in">
                    ...
                    <fo:table-cell text-align="right">
                        <fo:block> <@ofbizCurrency amount=(Static["org.sitenetsoft.sunseterp.applications.accounting.invoice.InvoiceWorker"].getInvoiceItemTotal(invoiceItem)) isoCode=invoice.currencyUomId!/> </fo:block>
                    </fo:table-cell>
                </fo:table-row>
            <#else>
...
<#if vatTaxIds?has_content>
 <fo:table>
    ...
    <#assign taxRate = delegator.findOne("TaxAuthorityRateProduct", Static["org.sitenetsoft.sunseterp.framework.base.util.UtilMisc"].toMap("taxAuthorityRateSeqId", vatTaxId), true)/>
    <fo:table-row>
        <fo:table-cell>
          <fo:block/>
        </fo:table-cell>
