...
import org.sitenetsoft.sunseterp.applications.product.store.ProductStoreSurveyWrapper;
...
public class ShoppingCartEvents {
    ...
    private static final String MODULE = ShoppingCartEvents.class.getName();
    ...
    public static String addToCart(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (UtilValidate.isEmpty(productId)) {
            ...
            if (UtilValidate.isEmpty(itemType)) {
                request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "cart.addToCart.noProductInfoPassed", locale));
        ...
        if ("ASSET_USAGE".equals(ProductWorker.getProductTypeId(delegator, productId))
            ...
            if (paramMap.containsKey("reservStart")) {
                ...
                if (!reservStartStr.isEmpty()) {
                    try {
                        ...
                        reservStart = null;
                        request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "cart.addToCart.rental.startDate", locale));
                        return "error";
                    }
                } else {
                    reservStart = null;
            ...
            if (paramMap.containsKey("reservEnd")) {
                ...
                if (!reservEndStr.isEmpty()) {
                    try {
                        ...
                        reservEnd = null;
                        request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "cart.addToCart.rental.endDate", locale));
                        return "error";
                    }
                } else {
                    reservEnd = null;
            ...
            if (reservStart != null && paramMap.containsKey("reservPersons")) {
                ...
                try {
                    ...
                    request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "OrderNumberOfPersonsShouldBeOneOrLarger", locale));
                    return "error";
                }
            }
        ...
        if ((ProductWorker.isAmountRequired(delegator, productId)) && (amount == null || amount.doubleValue() == 0.0)) {
            request.setAttribute("product_id", productId);
            request.setAttribute("_EVENT_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "cart.addToCart.enterAmountBeforeAddingToCart", locale));
            return "product";
        }
        ...
        if (UtilValidate.isNotEmpty(reserveAfterDateStr)) {
            ...
            try {
                reserveAfterDate = Timestamp.valueOf(reserveAfterDateStr);
            } catch (IllegalArgumentException e) {
    ...
    public static String quickInitPurchaseOrder(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (UtilValidate.isEmpty(billToCustomerPartyId) && UtilValidate.isEmpty(supplierPartyId)) {
            request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "OrderCouldNotInitPurchaseOrder", locale));
            return "error";
        }
    ...
    public static String addOrderTerm(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (UtilValidate.isEmpty(termTypeId)) {
            request.setAttribute("_ERROR_MESSAGE_", UtilProperties.getMessage(RES_ERROR, "OrderOrderTermTypeIsRequired", locale));
            return "error";
        }
    ...
    public static String setOrderCurrencyAgreementShipDates(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (UtilValidate.isNotEmpty(shipBeforeDateStr)) {
                ...
                cart.setDefaultShipBeforeDate(Timestamp.valueOf(shipBeforeDateStr));
            }
            if (UtilValidate.isNotEmpty(shipAfterDateStr)) {
                if (shipAfterDateStr.length() == 10) {
                    shipAfterDateStr += " 00:00:00.000";
                }
                cart.setDefaultShipAfterDate(Timestamp.valueOf(shipAfterDateStr));
            }
            if (UtilValidate.isNotEmpty(reserveAfterDateStr)) {
                if (reserveAfterDateStr.length() == 10) {
                    reserveAfterDateStr += " 00:00:00.000";
                }
                cart.setDefaultReserveAfterDate(Timestamp.valueOf(reserveAfterDateStr));
            }
            if (UtilValidate.isNotEmpty(cancelBackOrderDateStr)) {
                if (cancelBackOrderDateStr.length() == 10) {
                    cancelBackOrderDateStr += " 00:00:00.000";
                }
                cart.setCancelBackOrderDate(Timestamp.valueOf(cancelBackOrderDateStr));
            }
        } catch (IllegalArgumentException e) {
    ...
    public static String getConfigDetailsEvent(HttpServletRequest request, HttpServletResponse response) {
        ...
        request.setAttribute("totalPrice", org.sitenetsoft.sunseterp.framework.base.util.UtilFormatOut.formatCurrency(configWrapper.getTotalPrice(),
