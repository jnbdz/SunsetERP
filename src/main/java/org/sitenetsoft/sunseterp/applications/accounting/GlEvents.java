 ...
 * under the License.
 *******************************************************************************/
package org.sitenetsoft.sunseterp.applications.accounting;
...
public class GlEvents {
    ...
    public static String createReconcileAccount(HttpServletRequest request, HttpServletResponse response) {
        ...
        for (int i = 0; i < rowCount; i++) {  //for calculating amount per glAccountId
            ...
            try {
                GenericValue acctgTransEntry = EntityQuery.use(delegator)
                        .from("AcctgTransEntry")
                        .where("acctgTransId", acctgTransId, "acctgTransEntrySeqId", acctgTransEntrySeqId)
                        .queryOne();
        ...
        Map<String, Object> fieldMap = UtilMisc.toMap("glReconciliationName", "Reconciliation at date " + UtilDateTime.nowTimestamp(),
                "glAccountId",
                glAccountId, "organizationPartyId", organizationPartyId, "reconciledDate", UtilDateTime.nowTimestamp(),
                "reconciledBalance",
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            acctgTransEntrySeqId = (String) ctx.get("acctgTransEntrySeqId" + suffix);
            try {
                GenericValue acctgTransEntry = EntityQuery.use(delegator).from("AcctgTransEntry")
                        .where("acctgTransId", acctgTransId, "acctgTransEntrySeqId", acctgTransEntrySeqId)
                        .queryOne();
                if (acctgTransEntry != null) {
