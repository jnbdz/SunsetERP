...
public class RecordIterator {
    ...
    private boolean getNextLine() throws DataFileException {
        ...
        if (nextLine != null && !((nextLine.contains(eof)))) {
            ...
            if (isDelimited) {
                this.nextRecord = org.sitenetsoft.sunseterp.framework.datafile.Record.createDelimitedRecord(nextLine, nextLineNum, modelRecord, modelDataFile.getDelimiter(),
                        modelDataFile.getTextDelimiter());
            } else {
                this.nextRecord = org.sitenetsoft.sunseterp.framework.datafile.Record.createRecord(nextLine, nextLineNum, modelRecord);
            }
            return true;
        } else {
            this.close();
